...
(function () {
  const OWNER_EMAIL = 'hello@2600design.com.au';

  const FAQS = [
    {
      q: '💰 How much does a website cost?',
      a: 'Packages start from <strong>A$700</strong> for a Launch Website, <strong>A$1,700</strong> for a Business Website, and <strong>A$2,500</strong> for a Shopify store. Brand Identity starts from <strong>A$1,200</strong> and Digital Product UI from <strong>A$3,500</strong>. Every project is quoted individually — <a href="contact.html">get a free quote →</a>'
    },
    {
    ...
    },
    {
      ...
      a: 'Fill in the <a href="contact.html">contact form</a> or email me at <a href="mailto:hello@2600design.com.au">hello@2600design.com.au</a> — I\'ll get back to you within 24 hours!'
    }
  ];
  ...
  const css = `
    ...
    #chat-toggle {
      ...
      background: linear-gradient(135deg, #0c5364, #3f9292);
      border: 1px solid rgba(216, 239, 237, .45); cursor: pointer;
      display: flex; align-items: center; justify-content: center;
      box-shadow: 0 12px 28px rgba(6, 43, 58, .24);
      transition: transform .35s cubic-bezier(.34, 1.56, .64, 1), box-shadow .25s, filter .25s;
      position: relative;
    }
    #chat-toggle:hover {
      transform: translateY(-4px) scale(1.04);
      filter: saturate(1.08);
      box-shadow: 0 18px 34px rgba(6, 43, 58, .30);
    }
    ...
    #chat-dot {
      ...
      background: #8de0b0; border: 2px solid #f3f7f6;
    ...
    #chat-panel {
      ...
      width: 360px; max-height: min(620px, calc(100vh - 110px));
      background: rgba(243, 247, 246, .98);
      border: 1px solid rgba(45, 135, 144, .20);
      border-radius: 24px;
      box-shadow: 0 24px 70px rgba(6, 43, 58, .22);
    ...
    #chat-header {
      background: linear-gradient(135deg, #062b3a 0%, #0c5364 100%);
      padding: 20px 20px 18px;
      display: flex; align-items: center; gap: 12px; flex-shrink: 0;
    }
    #chat-avatar {
      width: 38px; height: 38px; border-radius: 50%;
      background: #9bd5d1; border: 1px solid rgba(255,255,255,.42);
      display: flex; align-items: center; justify-content: center;
      font-size: .9rem; font-weight: 700; color: #0c2530; flex-shrink: 0;
    }
    #chat-header-text { flex: 1; }
    #chat-name { font-size: .82rem; font-weight: 600; color: #f3f7f6; margin-bottom: 3px; }
    #chat-status { font-size: .6rem; letter-spacing: .07em; color: rgba(232, 247, 245, .68); display: flex; align-items: center; gap: 6px; }
    #chat-status::before { content:''; display:block; width:6px; height:6px; border-radius:50%; background:#8de0b0; box-shadow: 0 0 0 3px rgba(141,224,176,.14); }
    ...
    #chat-messages::-webkit-scrollbar-thumb { background: rgba(45, 135, 144, .24); border-radius: 4px; }
    ...
    .msg.bot {
      background: #fff; color: #0c2530; border: 1px solid rgba(45, 135, 144, .14);
      border-bottom-left-radius: 5px; align-self: flex-start;
      box-shadow: 0 5px 14px rgba(6, 43, 58, .05);
    }
    .msg.bot a { color: #247c87; }
    .msg.user { background: #247c87; color: #f3f7f6; border-bottom-right-radius: 5px; align-self: flex-end; }
    ...
    #faq-chips::-webkit-scrollbar-thumb { background: rgba(45, 135, 144, .24); border-radius: 3px; }

    .faq-chip {
      background: rgba(255,255,255,.72); border: 1px solid rgba(45, 135, 144, .18);
      border-radius: 20px; padding: 10px 14px;
      min-height: 44px;
      font-family: 'Montserrat', sans-serif;
      font-size: .74rem; color: #445b63; cursor: pointer;
      text-align: left; transition: background .25s, border-color .25s, color .25s, transform .3s cubic-bezier(.34,1.56,.64,1);
      white-space: normal; word-break: break-word;
      display: flex; align-items: center;
    }
    .faq-chip:hover { background: #fff; border-color: rgba(45, 135, 144, .46); color: #0c2530; transform: translateX(3px); }
    .faq-chip.active { background: rgba(155, 213, 209, .22); border-color: rgba(45, 135, 144, .46); color: #0c2530; }
    ...
    #week-selector-label {
      ...
      color: #2d8790; margin-bottom: 4px;
    }
    #week-buttons {
      display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px;
    }
    .week-btn {
      border: 1px solid rgba(45, 135, 144, .18); background: rgba(255,255,255,.72);
      border-radius: 8px; padding: 10px 4px;
      min-height: 44px;
      font-family: 'Montserrat', sans-serif;
      font-size: .72rem; color: #445b63; cursor: pointer;
      transition: background .2s, border-color .2s;
      text-align: center;
      display: flex; align-items: center; justify-content: center;
    }
    .week-btn:hover { background: #fff; border-color: rgba(45, 135, 144, .46); color: #0c2530; }

    /* Email form */
    #chat-email-section {
      border-top: 1px solid rgba(45, 135, 144, .14);
      padding: 14px 16px 16px; flex-shrink: 0;
      background: rgba(255,255,255,.58);
    }
    #chat-email-label {
      font-size: .6rem; letter-spacing: .14em; text-transform: uppercase;
      color: #2d8790; margin-bottom: 8px;
    }
    #chat-email-row { display: flex; gap: 8px; }
    #chat-email-input {
      flex: 1; border: 1px solid rgba(45, 135, 144, .20); border-radius: 999px;
      padding: 12px 14px; font-family: 'Montserrat', sans-serif;
      font-size: .78rem; color: #0c2530; background: #fff; outline: none;
      transition: border-color .2s; min-height: 44px;
    }
    #chat-email-input:focus { border-color: #2d8790; box-shadow: 0 0 0 3px rgba(45, 135, 144, .10); }
    #chat-email-input::placeholder { color: #91a5a8; }
    #chat-send-btn {
      background: linear-gradient(135deg, #247c87, #5eaaa7); border: none; border-radius: 999px;
      padding: 12px 16px; cursor: pointer; color: #f3f7f6;
      font-family: 'Montserrat', sans-serif; font-size: .72rem; font-weight: 600;
      letter-spacing: .04em; transition: transform .3s cubic-bezier(.34,1.56,.64,1), box-shadow .25s;
      white-space: nowrap; min-height: 44px;
    }
    #chat-send-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 18px rgba(22, 101, 111, .22); }
  ...
  widget.innerHTML = `
    ...
    <button id="chat-toggle" aria-label="Open chat">
      <div id="chat-dot"></div>
      <svg class="icon-chat" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#f3f7f6" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
        <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
      </svg>
      <svg class="icon-close" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#f3f7f6" stroke-width="2" stroke-linecap="round">
